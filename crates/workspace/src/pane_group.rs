...
impl PaneGroup {
    ...
    pub fn mark_positions(&mut self, cx: &mut App) {
        self.root
            .mark_positions(self.in_satellite, self.is_center, cx);
    }
...
impl Member {
    pub fn mark_positions(&mut self, in_satellite: bool, in_center_group: bool, cx: &mut App) {
        match self {
            Member::Axis(pane_axis) => {
                for member in pane_axis.members.iter_mut() {
                    member.mark_positions(in_satellite, in_center_group, cx);
                }
            }
